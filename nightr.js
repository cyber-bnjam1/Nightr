...
function showAuthScreen() {
  ...
  v.innerHTML = '<div class="vin" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:68vh;gap:22px;text-align:center;padding:0 24px;">' +
    '<div style="font-size:62px;">🌙</div>' +
    '<div><h2 class="vtitle">Bienvenue sur Nightr</h2><p class="vsub">Gère tes soirées comme un pro</p></div>' +
    '<button class="btn btn-p btn-full" style="max-width:280px;" id="btnGoogle">Continuer avec Google</button>' +
    '<button class="btn btn-s btn-full" style="max-width:280px;" id="btnAnon">Essayer sans compte</button>' +
    '<p style="font-size:12px;color:var(--txt3);max-width:250px;line-height:1.6;">Avec Google, tes soirées sont synchro sur tous tes appareils.</p></div>';
  document.getElementById('btnGoogle').onclick = function() {
    auth.signInWithPopup(new firebase.auth.GoogleAuthProvider()).catch(function(){ toast('Connexion annulée','err'); });
  };
  document.getElementById('btnAnon').onclick = function() {
    auth.signInAnonymously().then(function(){ toast('Mode invité activé','inf'); }).catch(function(){ toast('Erreur','err'); });
  };
}
...
function updateUserUI(user) {
  ...
  if (user.isAnonymous) { av.textContent='👤'; nm.textContent='Mode invité'; em.textContent='Données locales'; return; }
