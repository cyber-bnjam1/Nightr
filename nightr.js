...
// Proposer la connexion Google depuis les paramètres
function showLoginModal() {
  var close = showModal(
    '<h3 class="modal-title">🌙 Connexion</h3>' +
    '<p class="modal-sub">Connecte-toi avec Google pour synchroniser tes soirées sur tous tes appareils.</p>' +
    '<button class="btn btn-p btn-full" id="lg-google">Continuer avec Google</button>' +
    '<div style="height:10px;"></div>' +
    '<button class="btn btn-s btn-full" id="lg-cancel">Pas maintenant</button>'
  );
  on('lg-google','click', function() {
    auth.signInWithPopup(new firebase.auth.GoogleAuthProvider())
      .then(function(){ toast('Connecté avec Google !','ok'); close(); })
      .catch(function(){ toast('Connexion annulée','err'); });
  });
