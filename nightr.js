...
// Détecte mobile/Safari pour choisir popup vs redirect
function _isMobile() {
...
function showLoginModal() {
  ...
  on('lg-google', 'click', function() {
    ...
    if (_isMobile()) {
      // Mobile/Safari : redirect obligatoire (popup bloqué par iOS)
      ...
      // Desktop : popup (fluide, sans rechargement)
      var promise = (currentUser && currentUser.isAnonymous)
        ? currentUser.linkWithPopup(provider)
        : auth.signInWithPopup(provider);
      promise
        .then(function(result) {
          toast('Connecté ! Bienvenue ' + (result.user.displayName || '') + ' 🎉', 'ok', 4000);
          close();
        })
        .catch(function(e) {
          if (e.code === 'auth/credential-already-in-use' && e.credential) {
            auth.signInWithCredential(e.credential)
              .then(function(r) { toast('Connecté ! Bienvenue ' + (r.user.displayName || '') + ' 🎉', 'ok'); close(); })
              .catch(function() { toast('Erreur de connexion','err'); });
          } else if (e.code === 'auth/popup-closed-by-user' || e.code === 'auth/cancelled-popup-request') {
            toast('Connexion annulée','inf');
          } else {
            console.warn('Login error:', e.code, e.message);
            toast('Erreur : ' + (e.message || e.code), 'err');
          }
        });
    }
  });
